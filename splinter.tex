...
Similarly, businesses can leverage this data to extract
cross-user trends that would be difficult or impossible to
detect if users could not be compared to each other. 
Many of these applications currently allow users
to query these datasets. For example, users might search
...
are allowing also businesses and researchers to query
this data. Google, for instance, has an API to allow
businesses to query its map data to help them make
better business decision. However, the problem is that 
...
To solve these problems, we introduce Splinter, a practical system that allows servers
...
Then, the user\footnote{In this dissertation, we use the term "user"
to refer to a business, a researcher, and end-user of an application.} 
...
\label{sec:model}
There are three main principals in Splinter: the \emph{user}, 
the \emph{data owners}, and the \emph{providers}.
...
or license them from data owners. In both scenarios,
...
the results could be used to infer the source and destination. 
...
In this scenario, attempting a full download would be too expensive
...
is in cleartext and that the user communicates with each provider through a secure channel (e.g., using SSL),
